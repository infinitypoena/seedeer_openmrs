...
public class DailyScheduleGeneratorTests
{
    ...
    [Fact]
    public void Generate_ElPlanSoloTraeAltas()
    {
        // Los recurrentes NO se planifican: son la demanda del panel (quién tiene cita hoy, quién vuelve
        // por su cuenta) y depende de lo que haya pasado hasta hoy. Repartir el total con un porcentaje
        // fijo era el cupo que estranguló la agenda — ver leyes_simulacion.md.
        var gen  = new DailyScheduleGenerator(CreateSettings());
        var days = gen.Generate();

        foreach (var d in days)
            Assert.Equal(d.TotalPatients, d.NuevosPacientes);
    }
