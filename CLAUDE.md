...
- **Dynamic location & staff (consultorios + médicos)**: `ClinicResourceAssigner` (registered transient, called once per run from `SeedOrchestrator.RunAsync`) makes data realistic across multiple consultorios instead of a single "Unknown Provider"/stale location. Config `OpenMRS.Defaults.Consultorios[]` lists each consultorio's `LocationUuid` + a médico (`MedicoIdentifier` + `MedicoNombre`). `InitializeAsync` **idempotently** ensures each médico exists (`GET /provider?q={identifier}` exact-match → else `POST /person` then `POST /provider`). Per visit, `ProcesarVisitaAsync` calls `Assign()` → a random `(location, provider)` pair stored on `SimulatedPatient.AssignedLocationUuid/AssignedProviderUuid`; every seeder uses `patient.Assigned… ?? _settings.Defaults.…` (so empty `Consultorios` = old single-resource behavior). Patient registration uses `Defaults.RegistrationLocationUuid` (Recepción), not a consultorio. Médicos are reference data: `SIM-MED-*` identifiers, reused across runs, **not** voided by `DELETE /api/seed/clear`. Pure `ClinicResourceAssigner.Pick` is the testable seam. (Billing/cashier is NOT installed in this instance — out of scope.)
