...
- **Follow-up (nota de seguimiento)**: en `ConsultaSeeder`, con probabilidad `ReferralProbabilities.FollowUp` (def. 0.30) se registra una obs **fecha** "Return visit date" (`5096AAAA…`, datatype Date) con la próxima cita 7–30 días después de la visita. Antes este parámetro estaba en config pero ningún seeder lo usaba.
- **Resultados de laboratorio (ciclo orden→resultado)**: `LabOrderSeeder` captura el UUID de la orden (`POST /order`) y registra el resultado como **obs ligada a la orden** (`POST /obs` con campo `order`), coherente con la enfermedad. Seam puro `LabResultGenerator.Generar(lab, categorías, dxUuids, rng)`: **numérico** → banda normal (`res_min..res_max`) o anormal (`res_min_anormal..res_max_anormal`) si el paciente tiene una categoría de `res_trigger`; **codificado** → `res_normal_uuid` (p.ej. Negativo) vs `res_anormal_uuid` (Positivo), disparado por diagnóstico específico `res_trigger_dx` (p.ej. dengue→NS1 Positivo); **panel/imagen/vacío** → sin resultado (la orden queda sola). `ReferralProbabilities.LabResult` (def. 0.90) = fracción que "vuelve" con resultado el mismo día (resto pendiente, realista). v1 no cubre paneles (hemograma=`set`) ni imágenes. Testeado en `LabResultGeneratorTests`.
