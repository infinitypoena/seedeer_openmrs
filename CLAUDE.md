...
- **Dynamic location & staff (consultorios + médicos)**: `ClinicResourceAssigner` (registered transient, called once per run from `SeedOrchestrator.RunAsync`) makes data realistic across multiple consultorios instead of a single "Unknown Provider"/stale location. Config `OpenMRS.Defaults.Consultorios[]` lists each consultorio's `LocationUuid` + a médico (`MedicoIdentifier` + `MedicoNombre`). `InitializeAsync` **idempotently** ensures each médico exists (`GET /provider?q={identifier}` exact-match → else `POST /person` then `POST /provider`). Per visit, `ProcesarVisitaAsync` calls `AssignVisit(patient)`; every seeder uses `patient.Assigned… ?? _settings.Defaults.…` (so empty `Consultorios` = old single-resource behavior). **Médico de cabecera**: new patients draw a random consultorio that becomes their `CabeceraLocationUuid/CabeceraProviderUuid` (recurrent patients inherit it from the pool); a recurrent visit returns to that same médico with a **per-run probability** drawn uniformly in `[Simulation.MedicoCabeceraProbMin, Max]` (default 0.70–0.90, leans yes, like `CommonProbMin/Max`), else lands with another médico. Pure decision seam `ClinicResourceAssigner.UsarCabecera`. Patient registration uses `Defaults.RegistrationLocationUuid` (Recepción), not a consultorio. Médicos are reference data: `SIM-MED-*` identifiers, reused across runs, **not** voided by `DELETE /api/seed/clear`. Pure `ClinicResourceAssigner.Pick` is the testable seam. (Billing/cashier is NOT installed in this instance — out of scope.)
