...
| `diagnosticos.csv` | Query DB + manual + harvest + Centroamérica | CIEL diagnoses + age columns + `clima` + `cronica` + `comun` | ~950 dx, 13 categories (incluye enfermedades tropicales centroamericanas: dengue, chikungunya, zika…); harvested rows = peso 4; `cronica=true` → problem list; `comun=true` → pool de frecuentes |
