...
| 2026-07-17 | — | **Suite de tests reconstruida alrededor de la clínica (448 tests, 49 archivos).** Auditoría previa: la lógica del volumen está íntegra (suma altas+retornos, tripwire L3, aforo solo sobre altas) + corridas reales de validación de 1 semana contra OpenMRS (14 y 17 jul, exit 0, 0 errores, resultado idéntico — reproducibilidad confirmada). La suite vieja (~406 tests por acreción de piezas) se borró y se reconstruyó organizada por capas: **`Sistema/`** (el sistema corrido y juzgado por las leyes), `Modelo/`, `Clinica/`, `Laboratorio/`, `Catalogos/`, `Infraestructura/`, y **`TestSupport/`** (Escenarios · Factorias · Estadisticas · Azar · Repo · TempDir). La pieza clave es **`TestSupport/MiniClinica.cs`**: un harness que ejecuta la mecánica del día durante N años simulados **en memoria** reusando los seams puros reales (RecurrentSelector, Bass, DxDeControl, ClasificarCitas, SeguimientoPolicy, RecurrenceScheduler, SatisfaccionPolicy) y pasa `Invariantes.Evaluar` al final — la corrida sana de 3 años deja las 8 leyes verdes, y el sabotaje `CupoRecurrentes=0.30` (el bug histórico reintroducido a propósito) enciende L1/L2/L3/L7/L8 con 1,42 visitas/paciente (histórico: 1,45): **el fallo que pasó en verde ya no puede pasar en verde**. Verificación: 84/84 seams cubiertos (checklist por símbolo), 3 corridas de la suite idénticas, <10 s. Respaldo de la suite vieja en `backups/tests_viejos_20260714.zip` (gitignored) |
