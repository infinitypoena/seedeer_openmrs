...
| `Variedad.RepeticionDamping` | float (≥0) | Amortiguación anti-repetición: cada vez que un dx sale en la corrida su peso efectivo baja (`peso / (1 + damping × usos)`) → se explora la cola larga del catálogo (~950 dx). No altera el perfil por edad/sexo/clima ni los controles crónicos/agudos. `0` = apagado (def. 0.10 — bajado desde 0.25 en jul-2026 para que los cuadros comunes salgan más seguido; ↑ el valor da más variedad en la cola larga). |
